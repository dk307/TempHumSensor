...
#ifdef ESP8266_FORCE_S_MP_EXPTMOD
...
#ifdef ESP8266_S_MP_EXPTMOD_YIELD
  unsigned long yieldTime = millis();
  int cnt = 0;
#endif
  for (;;) {
#ifdef ESP8266_S_MP_EXPTMOD_YIELD
	  if (cnt++ > 20) {
		  unsigned long yieldNow = millis();
		  if (yieldNow - yieldTime > 500) {
			  yieldTime = yieldNow;
			  yield();
		  }
		  cnt = 0;
